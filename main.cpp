...
void my_Keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case 'w':
            ...
            break;
        case 's':
            data::game.getP()->move({-vback * Cos(data::game.getP()->getAng()), -vback * Sin(data::game.getP()->getAng())});
            break;
        case 'a':
            data::game.getP()->move({v * Cos(data::game.getP()->getAng()), v * Sin(data::game.getP()->getAng())});
            data::game.getP()->rotate(5);
            data::game.getP()->newAng(5);
            break;
        case 'd':
            data::game.getP()->move({v * Cos(data::game.getP()->getAng()), v * Sin(data::game.getP()->getAng())});
            data::game.getP()->rotate(-5);
...
void my_sKeyboard(int key, int x, int y) {
    ...
    switch (key) {
        ...
        case GLUT_KEY_RIGHT:
            ...
            break;
        case GLUT_KEY_HOME:
            data::game.getP()->moveto({(float)(x - 999) / 3.75f, (float)(y - 716.5) / -3.56f});
            break;
        case GLUT_KEY_END:
            data::game.getP()->setshoot(1);
            break;
